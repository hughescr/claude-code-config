...
- **A points band cannot be issued against an anchor nobody has defined** (v19, exit `2`). The
  id-then-text order deliberately leaves `v2` undefined between the two commands; what was missing
  was a guard on the gap, so a band could be issued against `v2` and its definition chosen
  *afterwards*, with the number already on disk and both tables append-only. `est open` now refuses
  in that window and names the command that closes it.
- **A migrated definition is a repair state, not a fact** (v19). The 17 → 18 step read the anchor's
  wording off two independently mutable config keys — the very pair that can say
  `{v1, "<the v2 definition>"}` — so it now lands as `sp_anchor.origin = 'unverified'`, and
  `sp_anchor_repair` is the way out: an append-only ledger with mandatory evidence, projected by
  `v_sp_anchor` with `MAX(seq)` winning, modelled on `estimate_identity_repair` rather than invented.
  The base row is never edited, so `SELECT text FROM sp_anchor` still returns what was believed at
  the time. `est anchor define <id> "<text>"` confirms or corrects it; once vouched for, it is
  immovable again.
- **Whether a conversion happened is a stored fact** (v19: `estimate.wcet_rate` /
  `estimate.wcet_rate_src`), not an inference from `cal_p50_wcet <> raw_p50_wcet`. The inference was
  wrong in exactly the way that matters: `pointsToWcet` is anchor-aware and refused the first band
  under a new anchor, but `est open` fell through to the anchor-*blind* `calibrationFor` and
  multiplied the band by the previous anchor's multiplier anyway — so `cal` differed from `raw`, the
  band read as "converted", and `est close` recorded a `velocity_cal` and an `in_band` from a rate
  the system had refused to issue. A refused rate now stores `cal = raw`, no refclass provenance, and
  `wcet_rate_src = 'none'`, which is what makes the row unscorable without anything having to be
  deduced. `'unrecorded'` marks pre-v19 rows, the only ones where the old reading is still consulted.
...
| Verb | What it does |
|---|---|
| `est refclass --text "<subject>" [--kind <k>] [--fanout <n>] [--full]` | The reference class, **shown before any number is stated** — top-5 FTS5 matches among completed tasks with their raw band, actual and velocity, plus one bucket line. Read-only, takes no lock, capped at 8,000 characters, and **always exits 0**: an empty reference class is a valid answer. Below 10 comparable tasks it prints the raw actual-cost distribution and **no multiplier**. Under `story_point` the **anchor leads** — `anchor <id>: "<text>" = 1 point`, plus the current points→Work-CET rate or a plain statement that there is none — because this is the only step that tells the estimator what "1" means; the rows and the distribution are already filtered to the estimand **and, under points, to the anchor** (v19), so a Work-CET actual never sits beside a points band with nothing saying so and a v1 match never sits under a v2 header; the bucket multiplier is likewise **withheld** whenever the newest snapshot is not fitted from this anchor's samples alone, because under points that multiplier *is* the Work-CET-per-point rate and showing it beside `NO RATE YET` put one anchor's number next to another anchor's name, and the footer's `unit:` line spells out that "velocity" then reads as Work-CET per point. `--fanout` narrows to a comparable agent fan-out as a **tolerance band** (half to double, minimum ±2), never equality, and falls back to the unfiltered class — saying so — rather than manufacturing an empty one. |
...
| `est retro [--as-of <iso>] [--dry-run]` | Weekly calibration panel plus the write-back that makes the ceremony non-inert: one `refclass` snapshot per bucket and one `calib_run` row, per `(ref_model, estimand)` partition so nothing is ever fitted across a unit change. `--dry-run` writes nothing and is the right habit while *n* is small. Under `story_point` the multiplier it fits **is** the points→Work-CET rate, since `velocity_raw = actual_wcet / raw_p50` with `raw_p50` in points. Note the gap while the points corpus is cold: `est close` NULLs `velocity_cal`/`in_band` when the calibrated band is still in points, and `scorePanel` / `blockPanel` consult the same `bandUnscorable` predicate, dropping those rows from the sample and counting them out loud rather than scoring them cross-unit. Since v19 that predicate reads the row's stored `wcet_rate_src` instead of comparing `cal_p50_wcet` to `raw_p50_wcet`. The multiplier fit is unaffected. After `est prices --fill-cw-1h`, `est retro` alerts `cw_ttl_repricing_boundary` if any closed outcome finalized before the fill is still in the corpus: a pure repricing event moves no `request` row, so that outcome's actual stays measured at the old, coarser rate while later ones are not, and the fit would otherwise pool both with no signal at all — hold `retro` until those outcomes are re-healed. |
...
| `est anchor list` (or no argument) · `est anchor define <id> "<text>" [--note <s>]` | v19: the story-point anchor registry, **by id**. `est config set sp_anchor_text` can only ever define the anchor *in force*, which left two reachable states with no way out through the CLI: a legacy id that appears only in `estimate.sp_anchor_id` (a band on disk naming a scale nobody wrote down — reaching it meant pointing `sp_anchor_id` at it, redenominating the live unit in order to annotate history), and an `'unverified'` definition the v18 migration read off the mutable config pair. `list` shows every definition with its state and names the undefined ids bands are still pointing at; `define` **declares** an id nobody has defined, **confirms or corrects** an unverified one by appending an `sp_anchor_repair` row (the recorded wording is kept — the correction lives beside it), and **refuses** (exit `2`) to re-word one that has been vouched for. The anchor in force never moves. |
