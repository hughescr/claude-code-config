...
| Path | What it is |
|---|---|
| `DESIGN.md` | The authoritative specification (rev. R3), LOCAL/untracked — see [Design](#design). Every `§` reference in this repo points here. |
| `DECISIONS.md` | Decision ledger: gate verdicts, policy rulings, calibration row zero, deliberate deferrals. |
...
| `scripts/` | Maintenance and operational scripts — **none of them installed**, see below. |
| `gates/` | Phase 0 pre-build gate probes and their reports (G-FORK, G-ATTR, G-PHASE, G-DELETE). |
...
| `spool/`, `backups/`, `sweep.lock` | OTLP spool, weekly `VACUUM INTO` backups, writer lock — all gitignored. |

The `gates/*.md` reports are committed evidence; the `gates/*.json` dumps they were derived from
are data, and are gitignored like the database (`DECISIONS.md` §2).
...
Phase 1 adds the rest of the `est` verbs — `open`, `block`, `bind`, `refclass`, `scope`, `burn`,
`close`, `retro`, `recon`, `nudge`, `capture-delete` — plus the hook wiring and the statusline
segment; Phase 2 adds the OTLP receiver and the board.
...
## Operational scripts — none of them are installed

`scripts/` holds the scheduled and hook-driven legs of the collector. **Every one of them is inert
until deliberately installed**, and installing them is a separate decision from building them.
Each file's own header is the install instructions; this table says only what exists and what its
state is.
...
| `session-end-sweep.sh` | **NOT WIRED IN** | The `SessionEnd` blocking sweep (`--budget 20s`, inside the 30 s hook timeout) so a session's own transcripts land in the DB before the process exits. It is not referenced by `~/.claude/settings.json`; the header explains how it must be added — as an **additional** user-level `SessionEnd` entry, never as a replacement, because hooks merge additively across settings levels and every level's hooks run. |
...
R3 §8 makes four probes blocking for Phase 1 logic. All four ran against the live corpus on
2026-07-28; the reports are in `gates/`, the decisions taken on them are in `DECISIONS.md` §1.

| Gate | Verdict |
|---|---|
| [G-FORK](gates/G-FORK.md) | **Prior falsified — keep the global `request_id` PK.** Overlap replicates: 5 independent pairs, 643 multi-file requestIds, 100% cross-session, `message.id` agreeing on all 643. Narrowing the key would double-count 2.4% of corpus Work-CET. The uuid-prefix fork detector is replaced by the shared-uuid detector; symlink aliasing and `/compact` continuations are newly documented mechanisms. |
| [G-ATTR](gates/G-ATTR.md) | **ESCALATED — sticky coverage is 18.7%, not the 82.8% the design assumes** (gate is 70%; the 82.8% was a mis-citation that counted `ambiguous` twice). The historical corpus is **not calibration-grade**: backfill seeds actual-cost distributions, never velocity. Live re-gate after ~2 weeks of real `est` usage. |
| [G-PHASE](gates/G-PHASE.md) | **No — run/agent grain with labelled inference is the default.** `agentId` join 87.7% against a 95% bar. The exact `workflowProgress` join is promoted to primary once cross-session run pairing and relaunch orphans are handled — **and the pairing fix landed after the gate ran, so re-run `gates/g-phase.ts` before Phase 1.** |
| [G-DELETE](gates/G-DELETE.md) | **Passed** — 12 of 12 `→deleted` events reconcile, 0 orphaned `TaskUpdate` calls of 562, and transcripts are primary. The `PreToolUse` capture hook is nevertheless **required** insurance: it is the only mechanism that survives a session dying between the `tool_use` and its `tool_result`. |
...
Last run (2026-07-28, TypeScript 7.0.2, suite green at 247 tests): **0 errors**. The twelve that
stood here before were all real drift between a caller and the code it calls, none reached at
runtime by any test, and all are fixed:
...
The authoritative specification is **`DESIGN.md`** (rev. R3, decisions applied) — kept LOCAL and
untracked (gitignored, alongside `DECISIONS.md` and the `gates/*.md` reports) because it contains
private prompt text and corpus-derived usage data, and this repository is public. Every `§`
citation in this repo points into it. Section 4.2 is the source for `schema.sql`; section 8
defines the phase boundaries and the pre-build gates. Where this code and that document disagree,
the document wins — or the document gets revised first. These local files are covered by the
weekly `backups/` snapshot job, not by git.

**R3 is superseded in part and will be revised to R4 before Phase 1.** Phase 0's gates falsified
three of its assumptions — the sticky-attribution coverage figure (G-ATTR: 18.7%, not 82.8%), the
narrow-the-`request`-PK prior (G-FORK: keep the global key), and the leading-uuid-prefix fork
detector (replaced by shared-uuid D3) — and turned up two mechanisms R3 does not describe at all
(symlink aliasing, `/compact` continuations). Until R4 lands, read R3 together with
[`DECISIONS.md`](DECISIONS.md) and `gates/*.md`, which record what actually held.
