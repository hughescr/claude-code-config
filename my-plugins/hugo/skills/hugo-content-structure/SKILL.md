...
- Legacy naming: pre-0.73 sites use `terms.html` (all terms) and `taxonomy.html` (one term). Hugo still applies that legacy meaning **inside `_default/` only** — and there a legacy `taxonomy.html` outranks a canonical `term.html` for term pages, so renaming within `_default/` breaks every term page. To modernise, move the files out of `_default/` to the top-level canonical names `layouts/taxonomy.html` (all terms) and `layouts/term.html` (one term), which dispatch correctly; per-taxonomy dirs (`layouts/<name>/`) likewise need the canonical names. Verified on Hugo v0.166 with `--templateMetrics`.
