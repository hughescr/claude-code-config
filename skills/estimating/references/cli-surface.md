...
| `sp_anchor_id` | `v1` | which `sp_anchor` row is in force. Pinned per estimate as `estimate.sp_anchor_id`, like `ref_model` and `price_epoch`; a redefinition changes what historic points mean, which is why the pin exists. Setting it to an id nobody has defined yet is allowed — that is the first half of introducing an anchor — but `est open` **refuses a points band (exit 2) until the second half lands**, because an anchor defined after the band is issued chooses what those points meant with the number already on disk |
| `sp_anchor_text` | *read it* | the work defined to be 1 point, for the id in force. Writing it **defines** that id in the append-only `sp_anchor` table; re-wording an id that is already defined is **exit 2**, because a re-wording is a redefinition. Introduce a new anchor id first, then its text. The config row itself is a mirror of the table — every reader resolves the table. It can only ever define the id **in force**; `est anchor define <id> "…"` is the per-id form, and the only way to state a definition for a legacy id or to confirm/correct one the v18 migration read off the old mutable config pair |
