...
Add the skill's `scripts/` directory to the path and import the `bsky` module:

```python
import sys
sys.path.insert(0, '/Users/craig/.claude/skills/browsing-bluesky/scripts')
# Portable form (resolve the .claude/skills symlink path for your environment):
#   sys.path.insert(0, '.claude/skills/browsing-bluesky/scripts')
import bsky
```

All functions are accessed as attributes of the `bsky` module — e.g.
`bsky.search_posts(...)`, `bsky.get_user_posts("handle", limit=15)`,
`bsky.get_profile(...)`. The available functions are:

```python
# Core browsing
bsky.search_posts, bsky.get_user_posts, bsky.get_profile, bsky.get_feed_posts, bsky.sample_firehose,
bsky.get_thread, bsky.get_quotes, bsky.get_likes, bsky.get_reposts,
bsky.get_followers, bsky.get_following, bsky.search_users,
# Trending
bsky.get_trending, bsky.get_trending_topics,
# Account analysis
bsky.get_all_following, bsky.get_all_followers, bsky.extract_post_text,
bsky.extract_keywords, bsky.analyze_account, bsky.analyze_accounts,
# Authentication utilities
bsky.is_authenticated, bsky.get_authenticated_user, bsky.clear_session
```

The examples below show bare function names for readability; call them as
`bsky.<function>` after the import above.
