...
| `gpt-astra-medium`, `gpt-astra-high`, `gpt-astra-xhigh` | OpenAI's frontier model, above sol. Cross-family peer of `fable-*`: only after sol stalls, or for exceptional judgment. |
| `deepseek-flash-low`, `deepseek-flash-medium`, `deepseek-flash-high` | The default DeepSeek route. Third-family peers of the Haiku, `sonnet-medium`, and `sonnet-high` rows; Flash at max effort ties sol at medium on the intelligence index. |
| `deepseek-v4-pro-low`, `deepseek-v4-pro-medium`, `deepseek-v4-pro-high` | Third-family peers of `sonnet-medium`, `opus-medium`, and `opus-high` by role only: on the index Flash scores higher at under half the cost, so use Pro only for a need the index does not measure. No image input. |

**The `gpt-*` and `deepseek-*` routes are unavailable unless `ANTHROPIC_BASE_URL` points at the local `utraque` proxy on `127.0.0.1:8317`; that key is not set by default.** `claude-smart.sh` (the `claude` shell alias) sets it automatically at launch when the utraque proxy answers healthy on `127.0.0.1:8317` — so from inside a session, check the `ANTHROPIC_BASE_URL` environment variable itself, not `settings.json`. With it unset, a `gpt-*` or `deepseek-*` spawn sends an unknown model name to `api.anthropic.com` and fails. See `UTRAQUE-SETTINGS-DELTA.md`, which documents the `settings.json` alternative.

When they are available, the `gpt-*` routes reach OpenAI models through that proxy and bill the Codex subscription; their context window is 272k tokens. The `deepseek-*` routes reach DeepSeek's Anthropic-compatible endpoint through the same proxy and spend a prepaid DeepSeek API balance; the proxy only advertises them when a DeepSeek key is configured. Effort reaches GPT models through the suffixed model name and reaches DeepSeek through the frontmatter `effort` key. Pair a non-Claude route with a Claude proposer for cross-family challenge; never make a `gpt-*` or `deepseek-*` route the only reviewer. If the proxy is configured but not running they fail immediately, and the Claude routes are unaffected.
